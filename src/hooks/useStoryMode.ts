...
const GRAVITY_ACTS: ActDef[] = [
  {
    ...
    // Low dramatic sweep from terrain level
    cameraStart: { pos: [4, 0.8, 4], target: [0, 0, 0] },
    cameraEnd: { pos: [2, 3, 4], target: [0, 0, 0] },
    fov: 60,
    actions: [{ progress: 0.15, fn: "setGravity" }],
    callouts: [{ label: "Mogul", worldPos: mogulPos, color: "#a855f7" }],
  },
  {
    ...
    narration: {
      ...
      body: "A gravitational signal propagates at 800 px/s — slower than light, faster than sound. Watch the terrain BUCKLE as the wavefront passes. Gravity waves don't bounce off mirrors or get muffled by walls. They warp the field that everything else moves through. The ground itself carries the message.",
    },
    // Start tight on Mogul, pull back as the DROP hits and terrain deforms
    cameraStart: { pos: [0.5, 1.2, 1.5], target: [0, 0.2, 0] },
    cameraEnd: { pos: [-1, 2.5, 3], target: [0, 0, 0] },
    fov: 55,
    actions: [{ progress: 0.3, fn: "broadcast" }],
    callouts: [{ label: "800 px/s — the ground carries the signal", worldPos: [0, 0.5, 0], color: "#a855f7" }],
  },
  {
    ...
    narration: {
      ...
      body: "These dense objects are Invariant Points — constitutional anchors with gravitational weight. Watch how they PIN the terrain while everything around them churns. They curve the signal field, creating wells that attract nearby agents. In governance, these are the non-negotiable principles: trust boundaries, escalation doctrines, rules that exist because removing them would collapse the system.",
    },
    // Low flyby past the mass — get intimate, see the terrain well
    cameraStart: { pos: [massPos[0] + 1.5, 0.8, massPos[2] + 1], target: massPos },
    cameraEnd: { pos: [massPos[0] - 0.5, 1.2, massPos[2] - 0.8], target: massPos },
    fov: 45,
    actions: [
    ...
    callouts: [
      { label: "Invariant Point — constitutional anchor", worldPos: massPos, color: "#a855f7" },
      { label: "Terrain pinned here ↓", worldPos: [massPos[0], massPos[1] + 0.5, massPos[2]], color: "#c084fc" },
    ],
  },
  {
    title: "The DROP",
    duration: 18,
    narration: {
      title: "Act 4 · The DROP",
      body: "Three gravity waves in rapid succession. Feel the terrain buckle under accumulated constitutional weight. Each wave compounds — the mesh deforms deeper, the wells pull harder, the lattice structure groans under the pressure of convergent invariance. This is what consensus looks like when it stops being polite.",
    },
    // Ultra-low terrain shot — camera barely above the mesh, watching it deform
    cameraStart: { pos: [1.5, 0.5, 0.5], target: [0, -0.1, 0] },
    cameraEnd: { pos: [-0.5, 0.6, -0.3], target: [0, -0.2, 0] },
    fov: 65,
    actions: [
    ...
    callouts: [{ label: "TERRAIN BUCKLING", worldPos: [0, 0.3, 0], color: "#ff44ff" }],
  },
  {
    title: "Field Warping",
    duration: 16,
    narration: {
      title: "Act 5 · Field Warping",
    ...
    // Tight orbit around the mass cluster — low altitude
    cameraStart: { pos: [0, 2, 0.1], target: [0, 0.2, 0] },
    cameraEnd: { pos: [0, 2, 0.1], target: [0, 0.2, 0] },
    orbit: { radius: 2.5, height: 1.8, center: [0, 0.2, 0] },
    fov: 55,
    actions: [
      { progress: 0.3, fn: "broadcast" },
      { progress: 0.7, fn: "broadcast" },
    ],
    callouts: [{ label: "Wavefront bends around invariants", worldPos: [0, 0.4, 0], color: "#a855f7" }],
  },
  {
    title: "The Pull",
    duration: 16,
    narration: {
      title: "Act 6 · The Pull",
      body: "Every mass exerts pull. Stand too close and the field bends you toward it. This is what constitutional gravity feels like — you don't choose to align. The topology chooses for you. Watch the horizon as you're dragged backward into the well.",
    },
    // Camera starts far, facing OUTWARD. Position moves toward mass. Target stays away. "Sucked in ass-first."
    cameraStart: { pos: [3, 1.5, 0], target: [6, 2, 1] },
    cameraEnd: { pos: [massPos[0] + 0.3, 0.8, massPos[2] + 0.3], target: [6, 2, 1] },
    fov: 55,
    actions: [{ progress: 0.4, fn: "broadcast" }],
    callouts: [{ label: "← You are being pulled", worldPos: [2, 1.2, 0], color: "#ff44ff" }],
  },
  {
    title: "The Weight of Consensus",
    duration: 16,
    narration: {
      title: "Act 7 · The Weight of Consensus",
    ...
    // Wide dramatic pullback — see the full warped terrain
    cameraStart: { pos: [0, 4, 6], target: [0, 0, 0] },
